...
const VacancyListItem = ({ item, deleteItem, addTab }) => {
    ...
    const { profession, url, office, date, quickly = false } = item;
        ...
        <div className="vacancy-list__item">
            <Link to={`/vacancies/${url}`} onClick={() => addTab(profession, `/vacancies/${url}`, office, 'Вакансии')}>
                ...
                <div className="vacancy-list__item__right">
                    ...
                    <div className="vacancy-list__item__date">{fireIcon} {date}</div>
                </div>
            </Link>
            <ContextMenu>
                <ContextMenuItem handleClick={() => console.log('edit ' + url)}
                                 icon={ <PencilIcon width={16} height={16} />}>
                    Изменить
                </ContextMenuItem>
                <ContextMenuItem handleClick={() => deleteItem(url)}
