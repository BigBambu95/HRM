...
import {
  ArchiveIcon, PencilIcon, RemoveBasketIcon, VacancyIcon,
} from '../../svg';
  ...
  return (
    <div>
      ...
      <Grid columns={1} gap="2em" style={{ marginTop: '2em' }}>
        {
                    items.map((item) => <CandidateListItem key={item.id} item={item} {...props} />)
                }
      </Grid>
    </div>
  );
...
const VacancyContainer = (props) => {
  const {
    candidates, reviewSummaryCandidates, phoneCandidates, interviewCandidates,
...
const mapDispatchToProps = (dispatch, ownProps) => {
  const {
    vacancyRequest, vacancyLoaded, vacancyError, removeVacancy, archiveCandidate, archiveAllCandidates,
  } = vacancies;
