...
import { getFilteredWorkers, getWorkerProfessions, getWorkerOffices, getWorkerDepartments, getWorker } from '../../selectors/workers';
...
class WorkerListContainer extends Component {
    ...
    componentDidMount() {
        const { openWorker, match } = this.props;


        console.log(match.params.id)
        if(match.params.id !== undefined) {
            openWorker(match.params.id);
        }
    ...
    render() {
        ...
        const {
            filteredWorkers, openWorker, isWorker, addTab,
            workerProfessions, workerOffices, workerDepartments,
            setFilterOfficeValue, setFilterProfessionValue, setFilterDepartmentValue, match
        } = this.props;
        ...
        const workerList = filteredWorkers.map(worker => {
            return <WorkerListItem key={worker.id} item={worker} match={match} addTab={addTab}  />
        });

        const itemList = filteredWorkers.length === 0
                        ? <p>По данным параметрам фильтрации не найдено результатов!</p>
                        : <Grid columns={columns} gap="2em">{workerList}</Grid>;
        ...
        return(
            <Fragment>
                <div className={clazz}>
                    <ToolBar>
                        <FilterList>
                            <Filter label="Должность" items={workerProfessions.concat('Все')} filter={setFilterProfessionValue} />
                            <Filter label="Офис" items={workerOffices.concat('Все')} filter={setFilterOfficeValue} />
                            <Filter label="Отдел" items={workerDepartments.concat('Все')} filter={setFilterDepartmentValue} />
                        </FilterList>
                        <ToolBarGroupItem>
                            <Button variant="outlined" color="purple">Выделить нескольких</Button>
                            <ContextMenu />
                        </ToolBarGroupItem>
                    </ToolBar>
                    <WorkerStatusPanel/>
...
const mapDispatchToProps = (dispatch, ownProps) => {
    ...
    const {
        workersRequest, workersLoaded, workersError,
        openWorker,
        setFilterOfficeValue, setFilterProfessionValue, setFilterDepartmentValue,
    } = actionWorkers;
