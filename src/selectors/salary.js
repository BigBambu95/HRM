...
export const getWorkersFinalSalary = createSelector(
  ...
  (getWorkersSalary, getWorkersOtherFees,
    ...
    getWorkersAccidentInsurance) => getWorkersSalary.map((item, idx) => item + getWorkersOtherFees[idx] - getWorkersIncomeTax[idx]
...
export const getWorkersTotalSalary = createSelector(
  ...
  (getWorkersFinalSalary) => getWorkersFinalSalary.reduce((sum, current) => sum + current, 0),
);
